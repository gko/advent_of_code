const evaluate = (expression = "") => {
	// console.log(expression, expression.search(/\d+\+\d+/));
	while (expression.search(/(\d+\+\d+)/) >= 0) {
...
const calculate = (expression = []) => {
	if (expression.length === 1 && typeof expression[0] === "string") {
	...
	return evaluate(
		expression.reduce(
			(expA, expB) =>
				(expA instanceof Array ? calculate(expA) : expA) +
				(expB instanceof Array ? calculate(expB) : expB),
			""
		)
	);
};
...
const parse = (expression = "") => {
	const trimed = expression.replace(/\s+/g, "");
