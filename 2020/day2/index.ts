const validatePart1 = (policy: string) => {
	const [criteria, password] = policy.split(":");
	const [, atLeast, atMost, ch] = criteria.match(/(\d+)-(\d+)\s+([a-zA-Z])/);

	const occurences = password.split("").filter((symbol) => symbol === ch)
		.length;

	return occurences <= Number(atMost) && occurences >= Number(atLeast);
};

const validatePart2 = (policy: string) => {
	const [criteria, password] = policy.split(":");
	const [, firstPosition, secondPosition, ch] = criteria.match(
		/(\d+)-(\d+)\s+([a-zA-Z])/
	);

	const passwordArray = password.split("");
	const firstContainsChar = passwordArray[Number(firstPosition)] === ch;
	const secondContainsChar = passwordArray[Number(secondPosition)] === ch;

	return [firstContainsChar, secondContainsChar].filter(Boolean).length === 1;
};
...
console.log(
	[
		...
		.map((password) => validatePart2(password))
